...
A minimalist daily habit tracker: a GitHub-style completion heatmap, a Pomodoro-style focus timer, daily reminders with browser notifications, and a light/dark theme — all backed by this small Node/Express server so your data survives restarts and isn't tied to one browser tab.

No account, no cloud service — everything is stored in a plain JSON file on your own machine.

This folder is the server half of the project; the client half lives in the sibling `../Frontend/` folder. `npm start` here serves both.
...
Then open **http://localhost:3000** in your browser. That's it — this one command starts the backend and serves the frontend files from `../Frontend/`.

Your data is saved to `Backend/data/store.json`, created automatically on first run. That folder is git-ignored, so cloning the repo never brings someone else's habit data with it, and your own data survives stopping and restarting the server.
...
- **Frontend:** vanilla HTML/CSS/JS in `../Frontend/` — no framework, no build step, no dependencies.
- **Backend:** Node.js + Express, serving `../Frontend/` and a small REST API (`/api/state`, plus a `PUT` endpoint per data collection).
...
```
Backend/
  server.js           Express server + REST API — serves ../Frontend and data/
  package.json
  data/store.json      Created automatically — your habit data (git-ignored)
Frontend/
  index.html           Frontend markup
  css/style.css         Styles (light + dark theme)
  js/script.js           Frontend logic — calls this server's API instead of localStorage
```
